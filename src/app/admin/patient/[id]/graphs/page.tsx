...
export default function AdminSugarGraphsPage() {
  ...
  // Rolling window ties to selection (matches dashboard)
  ...
  // Segment coloring by NEXT point (matches dashboard)
    ...
    <section className="space-y-4">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">
          Estimated HbA1c over time (rolling {windowDays}-day window).
        </p>
      </div>
      ...
      <section className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200/60">
        {a1cSeries.length === 0 ? (
          ...
          <div className="h-80 w-full">
            <ResponsiveContainer width="100%" height="100%">
              {/* Segmented coloring like dashboard; Area uses full series */}
              <LineChart data={segmentedSeries}>
                ...
                {/* Continuous segmented lines by NEXT point */}
                <Line
                  type="monotone"
                  dataKey="a1c_green"
                  stroke="#10b981"
        ...
        <p className="mt-3 text-xs text-slate-500">
          ...
          {Number.isFinite(overallA1c)
            ? ` Overall for selection ≈ ${format(overallA1c, 1)}%.`
            : ""}
        </p>
      </section>
